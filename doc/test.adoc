...
================
Number of threads
Number of clients (i.e. number of connections)
Size of payload
Echo or Ack
request lag on server
...
- minloglevel: run server with INFO logging turned off:
- echo_request_lag: number of milliseconds to wait before sending response
```
