...
The test works but the Test class is kind of useless.  A mental placeholder.  This is how the feature test is run:

[source, c++]
----
void Test::run() {
    eventBase_.runInEventBaseThread([http=httpClient_.get()]() {
        return http->connect()
...
Maybe something like this in the future?  Similar to link:https://github.com/cppfw/tst/blob/main/wiki/tutorial.adoc#declare-first-test-case[tst wiki]
